...
-- country has 20 rows with null values.
-- worlwide_gross_income has 3724 rows with null values.
-- Language column has 194 rows with null values.
-- production_company has 528 rows with null values.
*/
...
WHERE month(movie.date_published) = 3 AND
		   movie.year = 2017 AND
...
WHERE title like "The%" AND 
      avg_rating > 8
ORDER BY genre DESC, avg_rating DESC;
...
SELECT * FROM top5_grossing_movies
WHERE movie_rank <= 5;
...
SELECT 	dds.director_id as director_id,
...
GROUP BY dds.director_id, dds.director_name
ORDER BY count(dds.movieid) DESC, ROUND(AVG(dds.days_diff),2) DESC
LIMIT 9;   
